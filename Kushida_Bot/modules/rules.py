...
from Kushida_Bot.modules.console import rle
	...
	def stbtn(self,update,context):
		...
		if query.data == 's_btn':
			if group.id not in btns.keys():
				query.answer(text = "You Haven't set any button's yet! Please Set button using /setrulesbutton Command.", show_alert = True)
				return 
...
def setbutton(update,context):
	...
	if len(args) < 2:
		...
		if len(args1) == 3:
			...
			update.message.reply_text("<b>Button's Successfully Set for rules✅</b>", parse_mode = 'html')
...
def rule(update,context):
	...
	if chat.type == 'private':
		...
		return
	if chat.id not in rules.keys():
		update.message.reply_text('<b>Rules Not seted on this group. Ask admins for further infomation;)</b>', parse_mode = 'html')
		return 
...
def rmrul(update,context):
	...
	if chat.id not in rules.keys():
		update.message.reply_text("<b>You haven't Set any rules to delete! First Add rules then try this again.</b>", parse_mode = 'html')
		return 
	else:
		del rules[chat.id]
	...
	if chat.id in btns.keys():
		del btns[chat.id]
		del lnks[chat.id]
	update.message.reply_text("<b>All rulse has been successfully deleted✅</b>", parse_mode = 'html')
